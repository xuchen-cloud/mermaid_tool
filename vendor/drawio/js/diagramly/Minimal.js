...
EditorUi.initMinimalTheme = function()
{
	...
	function toggleFormat(ui, visible)
	{
		if (EditorUi.windowed)
		{
			...
			if (ui.formatWindow == null)
			{
				...
				ui.formatWindow.window.addListener(mxEvent.HIDE, mxUtils.bind(this, function()
				{
					notifyMinimalUiStateChanged(ui);
				}));
	...
	function toggleShapes(ui, visible)
	{
		if (EditorUi.windowed)
		{
			...
			if (ui.sidebarWindow == null)
			{
				...
				ui.sidebarWindow.window.addListener(mxEvent.HIDE, mxUtils.bind(this, function()
				{
					notifyMinimalUiStateChanged(ui);
				}));
	...
	function notifyMinimalUiStateChanged(ui)
	{
		if (ui != null && typeof ui.fireEvent === 'function')
		{
			ui.fireEvent(new mxEventObject('minimalUiStateChanged'));
		}
	}
    ...
    EditorUi.prototype.toggleFormatPanel = function(visible)
    {
		...
		notifyMinimalUiStateChanged(this);
    };

	var editorUiToggleShapesPanel = EditorUi.prototype.toggleShapesPanel;
	EditorUi.prototype.toggleShapesPanel = function(visible, noScroll)
	{
		editorUiToggleShapesPanel.apply(this, arguments);
		notifyMinimalUiStateChanged(this);
	};
	...
	EditorUi.prototype.init = function()
	{
		...
		function addMenuItem(label, fn, small, tooltip, action, img)
		{
            ...
            if (action != null)
            {
				...
				if (action.toggleAction && typeof action.isSelected === 'function')
				{
					updateSelectedState = function()
					{
						var isSelected = action.isSelected();
						btn.classList.toggle('geMenubarToggleButton-active', isSelected);
						btn.setAttribute('aria-pressed', isSelected ? 'true' : 'false');
					};

					action.addListener('stateChanged', updateSelectedState);
					ui.addListener('minimalUiStateChanged', updateSelectedState);
					updateSelectedState();
				}
            }
        ...
        function createGroup(btns, className, container)
        {
        ...
        function refreshMenu()
        {
			...
			var leadingControls = document.createElement('div');
			leadingControls.className = 'geMenubarPrimaryControls';
			var trailingControls = document.createElement('div');
			trailingControls.className = 'geMenubarSecondaryControls';
			var appElt = addMenu('diagram', null, (small) ? Editor.menuImage : null);
			var shapesElt = addMenuItem(mxResources.get('shapes'), ui.actions.get('toggleShapes').funct, null,
				mxResources.get('shapes'), ui.actions.get('toggleShapes'),
				(small) ? Editor.shapesImage : null);
			var formatElt = addMenuItem(mxResources.get('format'), ui.actions.get('format').funct, null,
				mxResources.get('format') + ' (' + ui.actions.get('format').shortcut + ')', ui.actions.get('format'),
				(small) ? Editor.formatImage : null);

			appElt.classList.add('geMenubarStandaloneButton');
			shapesElt.classList.add('geMenubarStandaloneButton', 'geMenubarToggleButton');
			formatElt.classList.add('geMenubarStandaloneButton', 'geMenubarToggleButton');

			leadingControls.appendChild(appElt);
			leadingControls.appendChild(shapesElt);
			leadingControls.appendChild(formatElt);

			var insertElt = addMenu('insert', true, (small) ? insertImage : null);
			var deleteElt = addMenuItem(mxResources.get('delete'), ui.actions.get('delete').funct,
				null, mxResources.get('delete'), ui.actions.get('delete'),
				(small) ? Editor.trashImage : null);

			insertElt.classList.add('geMenubarStandaloneButton');
			deleteElt.classList.add('geMenubarStandaloneButton');
			trailingControls.appendChild(insertElt);
			trailingControls.appendChild(deleteElt);
			...
			{
				createGroup([undoElt, redoElt], null, leadingControls);
				...
				if (iw >= 520 + embedOffset)
				{
					createGroup([fitElt,
						...
						(iw >= 640 + embedOffset) ? addMenuItem('', zoomOutAction.funct, true,
							...
							zoomOutAction, Editor.zoomOutImage) : null], null, leadingControls);
				}
			}
